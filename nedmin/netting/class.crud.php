...
class crud
{
    ...
    public function imageUpload($name, $size, $tmp_name, $dir, $file_delete = null)
    {
        ...
        try {
            ...
            if (!@move_uploaded_file($tmp_name, "dmg/$dir/$name_y")) {
                throw new Exception('Dosya Yükleme Hatası');
            }

            if (!empty($file_delete)) {
                unlink("dmg/$dir/$file_delete");
    ...
    public function adminsLogin($admins_username, $admins_pass, $remember_me)
    {
        try {
            ...
            if (isset($_COOKIE['adminsLogin'])) {
                $stmt->execute([$admins_username, md5(openssl_decrypt($admins_pass, "AES-128-ECB", "admins_coz")), 1]);
            } else {
                $stmt->execute([$admins_username, md5($admins_pass), 1]);
            }

            if ($stmt->rowCount() == 1) {
                $row = $stmt->fetch(PDO::FETCH_ASSOC);

                // admins dizisi oluşturuluyor ve kullanıcı bilgileri ekleniyor
                $_SESSION["admins"] = [
                    "admins_username" => $admins_username,
                    "admins_namesurname" => $row['admins_namesurname'],
                    "admins_file" => $row['admins_file'],
                    "admins_roles" => $row['admins_roles']
                ];
                $_SESSION["admins_id"] = $row['admins_id'];

                if (!empty($remember_me) and empty($_COOKIE['adminsLogin'])) {
                    // Eğer "Beni Hatırla" seçeneği işaretlendiyse, cookie oluşturuluyor
                    $admins = [
                        "admins_username" => $admins_username,
                    ...
                    // Eğer "Beni Hatırla" seçeneği işaretlenmediyse, cookie siliniyor

                    setcookie("adminsLogin", " ",  strtotime("-30 day"), "/");
                }
                return ['status' => TRUE];

                // Giriş başarılı olduğu için TRUE döndürülüyor

            } else {
                // Kullanıcı adı veya şifre yanlış olduğu için FALSE döndürülüyor

            }
            return ['status' => FALSE];
        } catch (Exception $e) {
            // Hata oluştuğunda hata mesajı döndürülüyor
            return ['status' => FALSE, 'error' => $e->getMessage()];
        }
    }
